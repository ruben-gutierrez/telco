...
function create_core_ims($names,$id_net,$domain,$typeDomain,$options_test_sprout){
	...
	foreach ($names as $nameVm=>$ipVm){
		...
		$transFile=shell_exec('scp -o "StrictHostKeyChecking no" -i /var/www/html/telco/scripts/terminal_testbed/key.pem /var/www/html/telco/scripts/test.sh ubuntu@'.$ipFloat.':/home/ubuntu');
		$ls=shell_exec('ssh -o "StrictHostKeyChecking no" -i /var/www/html/telco/scripts/terminal_testbed/key.pem ubuntu@'.$ipFloat.' "ls"');
		// shell_exec('ssh -o "StrictHostKeyChecking no" -i /var/www/html/telco/scripts/terminal_testbed/key.pem ubuntu@'.$ipFloat.' "sudo rm test.sh"');
		if( strpos($ls,'test') == false){
			shell_exec('sudo chmod 775 /var/www/html/telco/scripts/terminal_testbed/key.pem');
			$transFile=shell_exec('scp -o "StrictHostKeyChecking no" -i /var/www/html/telco/scripts/terminal_testbed/key.pem /var/www/html/telco/scripts/index.php ubuntu@'.$ipFloat.':/home/ubuntu');
		}
